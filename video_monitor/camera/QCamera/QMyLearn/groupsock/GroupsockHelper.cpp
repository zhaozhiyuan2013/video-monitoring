...
Boolean writeSocket(UsageEnvironment& env,
  ...
  do {
    MAKE_SOCKADDR_IN(dest, address.s_addr, port.num());
     //qDebug()<<"groupsockethelper Boolean writeSocket(UsageEnvironment& env, sendto(socket 329 ";
  ...
  if (ourAddress == 0) {
    ...
    do {
      ...
      int bytesRead = readSocket(env, sock,
				 ...
				 fromAddr);
       //qDebug()<<"fromAddr.sin_addr.s_addr"<<fromAddr.sin_addr.s_addr;
