...
Boolean H264or5VideoRTPSink::continuePlaying() {
  ...
  if (fOurFragmenter  == NULL) {
       //qDebug()<<" H264or5VideoRTPSink::continuePlaying() { 116   fOurFragmenter ";
 ...
 //qDebug()<<"H264or5VideoRTPSink::continuePlaying() {  125 ";
...
void H264or5VideoRTPSink::doSpecialFrameHandling(unsigned /*fragmentationOffset*/,
  ...
  // 2/ This NAL unit was the last NAL unit of an 'access unit' (i.e. video frame).
     //qDebug()<<" H264or5VideoRTPSink::doSpecialFrameHandling(un 136";
  if (fOurFragmenter != NULL) {
...
void H264or5Fragmenter::doGetNextFrame() {
  if (fNumValidDataBytes == 1) {
        //qDebug()<<"void H264or5Fragmenter::doGetNextFrame() if (fNumValidDataBytes == 1) { 180";
    // We have no NAL unit data currently in the buffer.  Read a new one:
    fInputSource->getNextFrame(&fInputBuffer[1], fInputBufferSize - 1,
			       afterGettingFrame, this,
			       FramedSource::handleClosure, this);
  } else {
       //qDebug()<<"void H264or5Fragmenter::doGetNextFrame() {   185 memmove(fTo, &fInputBuffer[1], ";
