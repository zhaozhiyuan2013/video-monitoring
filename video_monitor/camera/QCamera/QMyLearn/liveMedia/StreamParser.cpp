...
static int zzzzz= 0;
zzzzz++;
 //qDebug()<<"void StreamParser::ensureValidBytes1(unsigned numBytesNeeded) { zzzzz  "<< zzzzz;
     //qDebug()<<"  StreamParser::ensureValidBytes1(unsigned numBytesNeeded) {  126";
  ...
  unsigned maxNumBytesToRead = BANK_SIZE - fTotNumValidBytes;
   //qDebug()<<" StreamParser::ensureValidBytes1(   fInputSource->getNextFrame 156";
...
void StreamParser::afterGettingBytes1(unsigned numBytesRead, struct timeval presentationTime) {
    ...
    aaaaaaaaa++;
     //qDebug()<<" StreamParser::afterGettingBytes1(unsigned nu count  aaaaaaaaa = " << aaaaaaaaa;
